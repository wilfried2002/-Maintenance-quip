...
/**
 * Équipements de bureau. Tout le comportement générique (dashboard, interventions,
 * plans de maintenance, stock de pièces) vit dans le trait HandlesEquipementModule —
 * ce contrôleur ne garde que la spécificité bureau (catégorie, service affecté...).
 */
class EquipementBureauController extends Controller
{
    ...
    {
        $data = $request->validate($this->reglesValidation($equipementBureau->id));
    ...
    {
        return [
            'code' => ['required', 'string', 'max:50', 'unique:equipements_bureau,code' . ($id ? ",{$id}" : '')],
